...
$form['form-1'] = array(
	...
	'cfg' => array(
		'charset' => 'utf-8',
		'subject' => 'Новая заявка с сайта biaris.ru',
		'title' => 'Новый лид',
		'ajax' => true,
		'validate' => true,
		'from_email' => 'a.arhangelsky@gmail.com',
		'from_name' => 'lead@biaris.ru',
		'to_email' => 'a.arhangelsky@gmail.com',
		'to_name' => 'CEO biaris',
		'geoip' => true,
		'referer' => true,
		'type' => 'html',
...
if(isset($form[$act])) {
   ...
   $sb = array(); // subject и body
    ...
    if(count($error) == 0) {
      ...
      $get_fromEmail = (isset($form['fields'][$form['cfg']['from_email']]) && isset($getdata[$form['cfg']['from_email']]['value']) && mb_strpos('@', $getdata[$form['cfg']['from_email']]['value']) === false) ? $getdata[$form['cfg']['from_email']]['value'] : ((mb_strpos('@', $form['cfg']['from_email']) !== false) ? $form['cfg']['from_email'] : 'lead@'.$host);
